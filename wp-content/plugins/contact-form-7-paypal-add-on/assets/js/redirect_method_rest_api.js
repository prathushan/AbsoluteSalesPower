jQuery(document).ready(function($) {
	...
	function cf7pp_redirect(cf7pp_id, cf7pp_id_long) {
		...
		var endpoint = 'cf7pp_get_form_post';
		...
		jQuery.ajax({
			...
			url: ajax_object_cf7pp.ajax_url + endpoint,
			xhrFields: {
	...
	if (searchParams.has('cf7pp_stripe_success')) {
		...
		var stripe_success_endpoint = 'cf7pp_get_form_stripe_success';
		
		jQuery.ajax({
			type: "GET",
			dataType: "json",
			async: false,
			url: ajax_object_cf7pp.ajax_url + stripe_success_endpoint,
			xhrFields: {
				withCredentials: true
			},
			success: function (response) {
